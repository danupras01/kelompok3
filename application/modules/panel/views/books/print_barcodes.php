...
<link rel="stylesheet" href="<?= base_url();?>assets/dist/css/barcode_print.css">
<style type="text/css">
.padding05{
    padding-right: 5px;
}
        ...
        <div class="box-header with-border no-print">
          <h3 class="box-title"><i class="fa-fw fa fa-plus"></i><?= lang('print_barcode_title'); ?></h3><br>
            <small><?php echo sprintf(lang('print_barcode_heading'), 
                anchor('panel/books/categories', lang('category_label')),
                anchor('panel/books/authors', lang('author_label'))); ?></small>
          <div class="box-tools pull-right">
            <button type="button" class="btn  tip" onclick="customPrint();" id="print-icon" title="<?= lang('print_label'); ?>"><i class="icon fa fa-print"></i>
            </button>
          </div>
        </div>
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="well well-sm no-print">
                    ...
                    <?= form_open("panel/books/print_barcodes", 'id="barcode-print-form" data-toggle="validator"'); ?>
                    <div class="controls table-controls">
                        <table id="bcTable"
                               class="table items table-striped table-bordered table-condensed table-hover">
                            <thead>
                            <tr>
                                <th class="col-xs-8"><?= lang('book_title_label'); ?> (<?= lang('add_isbn_label'); ?>)</th>
                                <th class="col-xs-3"><?= lang('qty_label'); ?></th>
                        ...
                        <div class="form-group">
                         ...
                         <div class="form-group">
                            ...
                            <input name="author" type="checkbox" id="author" value="1" style="display:inline-block;" />
                            <label for="author" class="padding05"><?= lang('author'); ?></label>
                ...
                <div id="barcode-con">
                    <?php
                        if ($this->input->post('print')) {
                            if (!empty($barcodes)) {
                                echo '<button type="button" onclick="customPrint();" class="btn btn-primary btn-block tip no-print" title="'.'Print'.'"><i class="icon fa fa-print"></i> '.'Print'.'</button>';
                                $c = 1;
                                ...
                                foreach ($barcodes as $item) {
                                    for ($r = 1; $r <= $item['quantity']; $r++) {
                                        ...
                                        if($item['image']) {
                                            echo '<span class="product_image"><img src="'.base_url('assets/uploads/book_covers/'.$item['image']).'" alt="" width="20px"/></span>';
                                        }
                                        if($item['site']) {
                                        ...
                                        if($item['price']) {
                                            echo '<span class="barcode_price">'.lang('price_label').': ';
                                            echo $settings->currency. " " .$item['price'];
                                            echo '</span> ';
                                        }
                                        if($item['category']) {
                                            echo '<span class="barcode_category">'.lang('category_label').': '.$item['category'].'</span> ';
                                        }

                                        if($item['author']) {
                                            echo '<span class="barcode_category">'.lang('author_label').': '.$item['author'].'</span> ';
                                        }
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#add_item").autocomplete({
            source: '<?= site_url('panel/books/get_suggestions'); ?>',
            minLength: 1,
            autoFocus: false,
            delay: 250,
